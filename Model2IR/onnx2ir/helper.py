...
def fix_node_name(model):
    ...
    # type_index = {}
    c = 0
    # Record original node name -> new node name.
    updated_name_dict = {}
...
def get_npu_compatible_model(model,model_config,output_npu_model_filename):
    ...
    if type(graph_input_name) == list:
        if len(graph_input_name) > 1:
            raise ValueError("Only one input name is supported in model_config['input'][0].")
    graph_output_name = model_config["output"][0]
    if type(graph_input_name) == list:
        if len(graph_output_name) > 1:
            raise ValueError("Only one output name is supported in model_config['output'][0].")
    inputs = []
    outputs = []
    ...
    if graph_input_name not in inputs:
        raise ValueError(f"Input name not found in graph edges: {graph_input_name!r}")
    if graph_output_name not in outputs:
        raise ValueError(f"Output name not found in graph edges: {graph_output_name!r}")
...
def get_input_info(input_shape, name, ShapeType = False):
    '''
    input:
        input_shape: list of input shape dims
        name: current input node name
    return:
        t: input_info dict
    '''
    t = {}
...
def get_output_info(out_shape, ShapeType = False):
    '''
    input:
        out_shape: list of output dims
    return:
        output info list
    '''
    outputs_ = []
...
def get_weight_info(weight_shape,bias_shape=None):
    '''
    input:
        weight_shape: list of weight dims
    return:
        weight info dict
    '''
    weights_ = {}
