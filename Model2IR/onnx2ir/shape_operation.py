"""
Utilities for handling shape-changing operators.
"""
...
def load_onnx_model(model, label_file=None, output_npu_model_filename = None, store_intermediate_model=False, fix_layer_name=False):
    ...
    if node_output != []:
        ...
        for i in range(len(model.graph.input)):
            ...
            if len(input_dim) == 4:
                ...
                raise ValueError(f"Unsupported input shape for ORT probing: {input_dim}")
    ...
    if store_intermediate_model:
        # Pin ONNX IR/opset versions for compatibility.
...
def MeaninglessOpPass(model, constant_dict, initializer_dict):
    ...
    for node in model.graph.node:
        ...
        if node.op_type == 'Pad':
            ...
            if len(node.input) == 2:
                # The 2nd input is typically a constant node.
            ...
            if np.mean(pad_value) == 0 and np.std(pad_value) == 0:
                ...
                # Replace outputNode's 1st input with the original input; keep other inputs unchanged.
                inputs = []
            ...
            if node.input[1] in constant_dict.keys():
                # Second input is a constant node.
                c = constant_dict[node.input[1]]
                for attr in c.attribute:
                    # constant_dict[c.output[0]] = numpy_helper.to_array(attr.t)
...
def get_model_addr(model_attr):
    ...
    for a in model_attr:
        ...
        if a.name in ['group','ceil_mode','axis']:
            ...
            raise ValueError(f"Unsupported attribute field: {a!r}")
