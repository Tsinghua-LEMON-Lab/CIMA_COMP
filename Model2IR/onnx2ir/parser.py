...
class OnnxParser(object):
    ...
    def __init__(self, onnx_model,weight_half_level=None,
        ...
        # Keys differ; normalize to layer_name for weight_scale lookup.
                ...
                if weight_name in self.parameters.keys():
                    ...
                    if self.nodes[node].op_type == 'MatMul' and weight_name == self.nodes[node].input[1]:
                        ...
                        if len(numpy_helper.to_array(self.parameters[weight_name]).shape) >= 1:
                            # Keep consistent with pass-op naming.
                    ...
                    if node_name != None:
                        if weight_trans:
                            if '_lstm_x' in  node_name:
                                ...
                                data = data.squeeze()
                                data = data.transpose()
                                assert data.shape[1] % 4 == 0, f"Unsupported LSTM weight shape: {data.shape}"
                                len_ = data.shape[1] // 4
                                ...
                                data = numpy_helper.to_array(self.parameters[weight_name])
                                data = data.squeeze()
                                assert data.shape[0] % 8 == 0, f"Unsupported LSTM bias shape: {data.shape}"
                                len_ = data.shape[0] // 8
                        ...
                        if self.weight_scale != None and weight_name in self.weight_scale.keys():
                            ...
                            if self.weight_half_level != None:
                                raise ValueError(
                                    "Only one of {weight_scale, weight_half_level} is allowed. "
                                    "When weight_scale is provided, automatic scale computation is disabled."
                                )
