...
class MakeIROp:
    ...
    def Conv(self,ir,model_parser,node_name):
        ...
        # Detect whether bias is present.
        bias = False
        bias_shape = None
        if len(node.input) == 3:
        ...
        # By default conv has a single output which may be referenced by multiple layers.
    ...
    def Sub(self,ir,model_parser,node_name):
        op_ = make_op('sub')
        input_ = []
        node = model_parser.nodes[node_name]
        c = 0
    ...
    def Pad(self,ir,model_parser,node_name):
        ...
        if b'constant' not in mode:
            raise ValueError(f"Unsupported pad mode: {mode!r}. Only 'constant' is supported.")
        out_shape = dim_to_list(model_parser.value_infos[node.output[0]].type.tensor_type.shape.dim)
    ...
    def MatMul(self,ir,model_parser,node_name):
        # This op may have 1 or 2 outputs; outputs can be referenced by other layers.
        node = model_parser.nodes[node_name]
        weight = node.input[1]
        # Input shape.
        input_shape = dim_to_list(model_parser.value_infos[node.input[0]].type.tensor_type.shape.dim)
        ...
        # Check whether weight is a constant/initializer.
        if weight in model_parser.parameters.keys():
            weight_info = model_parser.value_infos[weight]
            weight_shape = dim_to_list(weight_info.type.tensor_type.shape.dim)
            inputs_ = [get_input_info(input_shape,ref_name)]
            # Swap axes so the first dim matches output dim.
            temp = weight_shape[1]
    ...
    def Reshape(self,ir,model_parser,node_name,batch_size):
        input_ = []
        node = model_parser.nodes[node_name]
        ...
        # Set batch dim to -1 for dynamic batch.
        if batch_size != None:
    ...
    def Resize(self,ir,model_parser,node_name):
        ...
        if len(node.input) == 3:
            if node.input[2] in model_parser.parameters.keys():
                if isinstance(model_parser.parameters[node.input[2]], numpy.ndarray):
                    scale = model_parser.parameters[node.input[2]].astype(np.int32).tolist()
                else:
    ...
    def Flatten(self,ir,model_parser,node_name):
        ...
        # This op is assumed to have a single input.
        input_shape = dim_to_list(model_parser.value_infos[node.input[0]].type.tensor_type.shape.dim)
        ref_name = model_parser.predecessors[node.input[0]][0].name
        # By default this op has a single output which may be referenced by multiple layers.
        out_shape = dim_to_list(model_parser.value_infos[node.output[0]].type.tensor_type.shape.dim)
        if ref_name in model_parser.inputs:
            index_ = model_parser.inputs.index(ref_name)
            ref_name = f'graph_input:{index_}'
    ...
    def Softplus(self,ir,model_parser,node_name):

        node = model_parser.nodes[node_name]
        op_ = make_op('softplus')
        input_ = []
        for i in node.input:
            input_shape = dim_to_list(model_parser.value_infos[node.input[0]].type.tensor_type.shape.dim)
            ref_name = model_parser.predecessors[i][0].name
    ...
    def Split(self, ir, model_parser, node_name):
        ...
        # ONNX opset differences: split may be stored in different fields.
        if split == None:
    ...
    def Gather(self,ir,model_parser,node_name):
        ...
        if node.input[1] in model_parser.predecessors.keys():
            ...
            raise ValueError("Gather indices are unknown: cannot find indices in constants/initializers.")
    ...
    def Slice(self, ir, model_parser, node_name):
        ...
        t = get_input_info(input_shape,ref_name,ShapeType=True)
        input_.append(t)
        output_shape = dim_to_list(model_parser.value_infos[node.output[0]].type.tensor_type.shape.dim)
        output_ = get_output_info(output_shape, ShapeType=True)
    ...
    def Expand(self, ir, model_parser, node_name):
        node = model_parser.nodes[node_name]
        ...
        shape = numpy_helper.to_array(model_parser.parameters[node.input[1]]).tolist()
        op_ = make_op('expand', shape=shape)
